...
**Advanced**: Mathematically, the values in `pop` are integers and there is an integer class in R. However, by default, numbers are assigned class numeric even when they are round integers, for example `class(1)` returns numeric. You can turn them into class integer with `as.integer()` or by adding an `L` like this: `1L`. Note the class by typing: `class(1L)`.
