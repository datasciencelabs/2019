...
So, for example, the correlation between a variable and itself is 1 and the correlation between a variable and its negative is -1.  For other pairs, the correlation is between -1 and 1. For instance, the correlation between father and son's heights is about 0.5:
...
If there is perfect correlation, we predict an increase that is the same number of SDs. If there is 0 correlation, then we don't use $x$ at all for the prediction and simply predict the average $\mu_Y$.  For values between 0 and 1, the prediction is somewhere inbetween. If the correlation is negative, we predict a reduction instead of an increase. For negative values, we predict in the opposite direction.
...
$$
 \mbox{ with slope } m = \rho \frac{\sigma_y}{\sigma_x}
$$

$$
 \mbox{ and intercept } b=\mu_y - m \mu_x
$$
...
1. Round father heights to closest inch, stratify, and then take the average.
...
Specifically, it is reduced to $\sqrt{1-\rho^2} = \sqrt{1 - 0.25}$ = 0.86 of what it was originally. We could say that the father's height "explains" 14% of the son's height variability. 
...
Now let's turn our attention back to the original problem: predicting birthweight. Since participation in the First Steps program in dichotomous, let's turn our attention to the gestational age variable. First, notice that the birthweight and gestational age data appear to be fairly bivariate normal:
...
Now we are ready to use linear regression to predict birthweight if we know the gestational age of the infant. All we need to do is compute the five summary statistics:
...
and use the formulas given above to create the regression line:

```{r}
reg_line <- summary_stats %>% summarize(slope = r*s_bwt/s_gest,
                              intercept = avg_bwt - slope*avg_gest)

p + geom_abline(intercept = reg_line$intercept, slope = reg_line$slope, color = "blue")
```
...
In the example above, the slope is `r reg_line$slope`. So this tells us that for every additional week of gestation, the average birthweight increases by `r reg_line$slope` grams.
